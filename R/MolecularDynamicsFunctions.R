...
ripley_target_euclidean_main <- function(points, ROI, R, lambda, number_of_points){
  ...
  lambda <- prod(ROI) / number_of_points
  Ks <- lambda * Ks
  targets <- vector("list", number_of_points)
  for(i in 1:number_of_points){
    targets[[i]] <- cbind(R, Ks[,i])
...
#' @param target Manual input for target function. Can be left as NULL as long as a target point cloud is provided.
...
#' @param nrval Numeric value, the number of equally-spaced radial values for each K function to be calculated over.
#' @param dimensions Dimensions of data set.
#' @param targets Manual input for target function. Can be left as NULL as long as a target point cloud is provided.
#' @param number_of_points Number of points to use in simulation.
#' @param initial_distribution The initial frame of the simulation, used to specify pre-determined spatial organisation. If left NULL, a uniformly random distribution will be used.
#' @param output_target Boolean, determines whether spatial statistics will be output alongside point cloud at each frame. Useful for tracking changes in spatial statistics.
#' @return A molecular dynamics simulation. This takes the form of a list, containing a point cloud for each time frame. Will also contain spatial statistics at each time frame if output_target is TRUE.
...
#' @param nrval Numeric value, the number of equally-spaced radial values for each K function to be calculated over.
#' @param dimensions Dimensions of data set.
#' @param targets Manual input for target functions. Can be left as NULL as long as target point clouds are provided.
#' @param number_of_points Number of points to use in simulation.
#' @param initial_distribution The initial frame of the simulation, used to specify pre-determined spatial organisation. If left NULL, a uniformly random distribution will be used.
#' @param output_target Boolean, determines whether spatial statistics will be output alongside point cloud at each frame. Useful for tracking changes in spatial statistics.
...
#' @param ROI A vector of two values representing the width and height of the region of interest, respectively.
#' @param times Number of time steps or iterations of the simulation.
#' @param point_clouds A list of points data, where each data set is given as a matrix or data frame with three columns: x coordinate, y coordinate, and population number of each point. Used as target distributions from which spatial statistics are drawn.
#' @param rmax Numeric value, the maximum radius for each K function to be calculated at.
#' @param nrval Numeric value, the number of equally-spaced radial values for each K function to be calculated over.
#' @param dimensions Dimensions of data set.
#' @param initial_distribution The initial frame of the simulation, used to specify pre-determined spatial organisation. If left NULL, a uniformly random distribution will be used.
...
animate_points <- function(simulation, integer_times = TRUE, time_step = 1, time_unit = "ms", axis_text_size = 12){
  ...
  p <- ggplot2::ggplot(all_data, ggplot2::aes(x = x, y = y)) +
    ...
    ggplot2::theme(panel.background = ggplot2::element_blank(), panel.grid.major = ggplot2::element_blank(),
                   panel.grid.minor = ggplot2::element_blank(), axis.text.x = ggplot2::element_text(size = axis_text_size),
                   axis.text.y = ggplot2::element_text(size = axis_text_size), axis.title.x = ggplot2::element_blank(),
...
#' @export
animate_populations <- function(simulation, integer_times = TRUE, time_step = 1, time_unit = "ms", labels = NULL, text_size = 16, axis_text_size = 12, point_size = 1){
...
#' @export
animate_target <- function(simulation, ROI, target, rmax = 200, nrval = 20, integer_times = TRUE, colours = c("red", "black"), labels = c("r", "K(r)"), time_step = 1, time_unit = "ms", axis_text_size = 16, title_text_offset = 4){
  ...
  p <- ggplot2::ggplot(all_data, ggplot2::aes(x = x)) +
    ...
    ggplot2::theme(panel.background = ggplot2::element_blank(), panel.grid.major = ggplot2::element_blank(),
                   ...
                   axis.text.x = ggplot2::element_text(size = axis_text_size), axis.text.y = ggplot2::element_text(size = axis_text_size),
                   axis.title.x = ggplot2::element_text(size = axis_text_size + title_text_offset), axis.title.y = ggplot2::element_text(size = axis_text_size + title_text_offset),
                   legend.text = ggplot2::element_text(size = axis_text_size), legend.title = ggplot2::element_blank(),
...
#' @export
animate_H_from_points <- function(simulation, ROI, point_clouds, rmax = 200, nrval = 20, integer_times = TRUE, colours = c("red", "black"), labels = c("r", "Target", "Actual"), time_step = 1, time_unit = "ms", time_points = NULL){
  #Create point clouds list.
  if(is.data.frame(point_clouds)){
  ...
  if(length(point_clouds) > 1){
    ...
    if(is.null(time_points)){
      ...
      p <- ggplot2::ggplot(all_data, ggplot2::aes(x = x)) +
        ...
        ggplot2::geom_line(ggplot2::aes(y = Target, colour = "Target")) +
        ggplot2::labs(x = labels[1], y = labels[2]) +
        ggplot2::scale_colour_manual(name = "Functions", values = cols) +
        ggplot2::scale_x_continuous(expand = c(0.01, 0.01)) +
        ggplot2::scale_y_continuous(expand = c(0.01, 0.01)) +
...
#' @return A gganimate object representing the animated K function and target statistic.
#' @export
animate_K_from_points <- function(simulation, ROI, point_clouds, rmax = 200, nrval = 20, integer_times = TRUE, colours = c("red", "black"), labels = c("r", "Target", "Actual"), time_step = 1, time_unit = "ms", time_points = NULL){
  #Create point clouds list.
  if(is.data.frame(point_clouds)){
    point_clouds <- list(point_clouds)
  }
...
#' @return A gganimate object representing the animated H function and target statistic.
#' @export
animate_H_from_populations <- function(simulation, ROI, point_cloud, population_number, rmax = 200, nrval = 20, integer_times = TRUE, colours = c("red", "black"), labels = c("r", "Target", "Actual"), time_step = 1, time_unit = "ms"){
  #Get targets.
